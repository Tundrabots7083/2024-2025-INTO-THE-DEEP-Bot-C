...
public class AscentMotorTest extends OpMode {
    ...
    public void loop() {
        ...
        if(LINEAR_SLIDE_LENGTH != lastLinearSlideLength) {
            ascentMotor.setLength(LINEAR_SLIDE_LENGTH);
            lastLinearSlideLength = LINEAR_SLIDE_LENGTH;
        }

        ascentMotor.execute();
        telemetry.addData("Target Length", ascentMotor.getTargetLength());
        telemetry.addData("Current Length", ascentMotor.getCurrentLength());
        telemetry.update();
    }
