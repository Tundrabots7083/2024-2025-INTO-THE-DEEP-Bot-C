...
import java.util.List;
    ...
    class ColorBlobLocatorProcessorImplEx extends ColorBlobLocatorProcessorEx implements VisionProcessor
    {
        private ColorRangeEx colorRange;
        private ColorRangeEx colorRangeEx;
        private ImageRegionEx roiImg;
        private Rect roi;
        ...
        private Mat additionalMask = new Mat();
        ...
        ColorBlobLocatorProcessorImplEx(ColorRangeEx colorRange, ImageRegionEx roiImg, ContourMode contourMode,
        ...
        ColorBlobLocatorProcessorImplEx(ColorRangeEx colorRange, ColorRangeEx colorRangeEx, ImageRegionEx roiImg, ContourMode contourMode,
                                        int erodeSize, int dilateSize, boolean drawContours, int blurSize,
                                        @ColorInt int boundingBoxColor, @ColorInt int roiColor, @ColorInt int contourColor)
        {
            this.colorRange = colorRange;
            this.colorRangeEx = colorRangeEx;
            this.roiImg = roiImg;
            this.drawContours = drawContours;
            this.boundingBoxColor = boundingBoxColor;
            this.roiColor = roiColor;
        ...
        public Object processFrame(Mat frame, long captureTimeNanos)
        {
            ...
            Core.inRange(roiMat_userColorSpace, colorRange.min, colorRange.max, mask);
             if(colorRangeEx != null) {
                Core.inRange(roiMat_userColorSpace, colorRangeEx.min, colorRangeEx.max, additionalMask);
                Core.add(mask, additionalMask, fullMask);
            }

            if (erodeElement != null)
            {
                Imgproc.erode( colorRangeEx == null ? mask : fullMask, colorRangeEx == null ? mask : fullMask, erodeElement);
            }

            if (dilateElement != null)
            {
                Imgproc.dilate(colorRangeEx == null ? mask : fullMask, colorRangeEx == null ? mask : fullMask, dilateElement);
            }

            ArrayList<MatOfPoint> contours = new ArrayList<>();
            Mat hierarchy = new Mat();
            //Imgproc.findContours(mask, contours, hierarchy, contourCode, Imgproc.CHAIN_APPROX_SIMPLE);
            Imgproc.findContours(colorRangeEx == null ? mask : fullMask, contours, hierarchy, contourCode, Imgproc.CHAIN_APPROX_SIMPLE);
            hierarchy.release();
