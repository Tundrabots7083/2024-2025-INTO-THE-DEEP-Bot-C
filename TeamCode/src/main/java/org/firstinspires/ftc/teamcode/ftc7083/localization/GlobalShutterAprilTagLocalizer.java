 ...
 */
@Config
public class GlobalShutterAprilTagLocalizer implements Localizer {
    ...
    private final GlobalShutterCamera globalShutterCamera;
     ...
     */
    public GlobalShutterAprilTagLocalizer(GlobalShutterCamera globalShutterCamera) {
    ...
    /**
     * Updates the pose of the robot using backwards facing global shutter camera on the robot.
     * It gets the April Tags detected by the camera and determines the pose
     * and orientation of the robot. The pose is then averaged over all April Tag detections
     * for each detection.
     */
    @SuppressLint("DefaultLocale")
    private void updatePose() {
        ...
        int numDetections = 0;
            telemetry.addLine(globalShutterCamera.toString());

            List<AprilTagDetection> detections = globalShutterCamera.getAprilTagDetections();
